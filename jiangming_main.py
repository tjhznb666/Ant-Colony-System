    ...
    if NC >= T - 1:
        ...
        if justice2:
            for p in Best_way[Num_of_best]:
                Pheromone[p[0]][p[1]] = eta * Q / L_shortest + Pheromone[p[0]][p[1]]
...
x = []
y = []
for l in range(len(new_way)):
    x.append(new_way[l][1]+0.5)
    y.append(mapsize[0]-new_way[l][0]-0.5)
plt.plot(x,y)
